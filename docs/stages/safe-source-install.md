...
Focused offline tests cover shell-config ambiguity, unsupported versions, and host-coerced ID types; stale enabled locations; fixed interpreter paths without bytecode writes; assume-unchanged and skip-worktree rejection; shallow, sparse, promisor, alternate-object, common-directory, missing-object, and gitlink rejection; no explicit rescan; stopped-state rechecking immediately before rename; real no-replace renames; an existing target; a target raced into place at the syscall boundary and through the complete recovery path; delayed startup discovery; one enable; checkout restoration before enablement; active and uncertain helper retention; disable-with-tree-retention after an attempted enable; persisted disable verification; and shell stop when disablement fails.
