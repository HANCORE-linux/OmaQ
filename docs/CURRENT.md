...
Commit `ef543c0` passed the full unit/IPC, helper, architecture, lock/multi-client/multi-home, phase 2–6 and 8, EncryptSave, Ratchet-restart, no-Signal, core-QML, plugin, syntax, and focused Wayland matrix; both live installations passed checksum and plugin validation and run protocol 8. The current compact-frame follow-up passes unit/IPC tests, native helper compilation, core QML lint, plugin and asset validation, diff/syntax checks, and focused Quickshell runtime contracts for all rail states, pending controls, borderless support focus, and fixed 48-pixel headers. Its independent final QML review reports exactly `0 findings`. Native separate-network acceptance remains open; `qmllint Panel.qml` still exits 255 without diagnostics.
