...
- **Notifications and appearance:** the fixed 400-pixel panel follows Omarchy, while chat windows support the system palette and bundled themes. Message size, formatting tools, the bundled UHOH notification, other sounds, custom bounded PCM WAV imports, global Mute, per-conversation **Pop up: On/Off**, badges, and notification surfaces remain configurable. Floating windows keep their full drag area without displaying a drag glyph.
