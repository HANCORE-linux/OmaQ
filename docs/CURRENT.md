...
- **Source commit:** local `main` contains two commits after `origin/main`; the remote remains at `a8a5af5`
- **Live machine:** local UI preview includes `a8a5af5`; deployed helper SHA-256 `16115b41cdacab6c575eed2c5c180abca828f33964bbd238d3413d6d1fd4ac6e`
- **Live machine2:** UI remains at `8e36ab6` on `192.168.2.108`; deployed helper SHA-256 `16115b41cdacab6c575eed2c5c180abca828f33964bbd238d3413d6d1fd4ac6e`
...
- **Files:** paused offer, explicit Accept/Decline, cancelable outgoing transfers, default downloads under `~/Downloads/omaq/`, fully decoded and canonically rewritten bounded avatars, and in-chat playback for received audio files without image or video previews.
...
The committed follow-up on top of `6abef09` passed unit/IPC and sanitizer tests, helper and no-Signal builds, architecture checks, lock/multi-client/multi-home tests, phases 2–6 and 8, EncryptSave, Ratchet restart, plugin validation, core QML lint, shell syntax, and `git diff --check`. Phase 3 failed once while waiting for an external Tox request and passed immediately when rerun in isolation; no sanitizer or code failure accompanied it. Focused regressions cover stable direct IDs, strict restart-safe prekeys and response replay, wrong-peer rejection, failed session rollback, malformed or stale records, bounded canonical avatars including JPEG/WebP downscaling and legacy cleanup, file cancellation, identity markers, exact-window focus, multi-monitor watcher takeover, settings persistence verification, and uninstall reporting. Independent final AppSec/crypto and QML/Omarchy reviews both report `0 findings`. The local live preview still includes only `a8a5af5`; Machine2 remains on `8e36ab6`. Native separate-network and live Wayland acceptance remain open; `qmllint Panel.qml` still exits 255 without diagnostics.

## Next order

1. Push and deploy only after fresh approvals for each delivery phase.
