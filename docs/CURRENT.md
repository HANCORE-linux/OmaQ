...
- **Notifications:** `Auto-open` is per conversation and reports its on/off state in the tooltip; the right-side panel rail exposes Invite, Add, Chat, Groups, Search, Identity, Theme, Sounds, Demo, Mute, and Danger without button tiles; unread counts remain until the relevant chat is actively opened.
