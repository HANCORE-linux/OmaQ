...
The short source-install command is a convenience path that trusts the user's Git configuration and environment. URL rewrites, proxies, credential helpers, and TLS settings can change its acquisition behavior before the downloaded `install.sh` runs; later checkout validation cannot undo that pre-execution trust. Use the bounded exact-commit bootstrap in the installation guide when acquisition must ignore user Git configuration and bind reviewed code before execution.
