...
6. Pure policy has no IO: `roles.c`, `invite.c`, `conversation.c`. IO lives in focused helper modules: `store.c` owns history, `surface.c` owns `surfaces.jsonl`, `auto_open.c` owns active Auto-open preferences, and `state_archive.c` owns non-overwriting private state copies. `tox_adapt.c` owns (`tox.h` + `toxav.h` + `toxencryptsave.h` only). Signal/Olm headers live only in `ratchet_adapt.c`.
