...
Settings (phase 4): `notifyBadge`, `notifyRightPanel`, `notifyDesktop`, `surfaceMode` (`separate`|`bundled`), `sound`, `soundCustomPath`, `chatTheme`, `messageScale` (`0.85`–`1.2` in five fixed steps), `animateUnread`. The compact framed panel keeps green/gray friend status dots, lists active groups with member counts beneath Friends, uses slim theme-radius action buttons, and arranges its tileless action icons in two framed vertical columns. Selecting a listed group opens its chat directly. The group-chat header can invite an existing contact, and message scaling affects message bodies only.
