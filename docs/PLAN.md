...
| Sound | Compact Settings picker: off, UHOH, PING, MAIL, Aurora, Glow, Click, Knock, and user-imported files. UHOH uses `sounds/icq-message.mp3`; its stable setting id remains `icq-message`. Users may remove only OmaQ-managed custom copies; bundled sounds remain immutable. Removed preset selections fall back to UHOH. |
...
{"op":"call.start","conversation":"...","key":"64-hex-direct-key","request":"client-request-id"}
{"op":"call.answer","conversation":"...","key":"64-hex-direct-key","callId":"16-hex-local-call-id","request":"client-request-id"}
{"op":"call.stop","conversation":"...","key":"64-hex-direct-key","callId":"16-hex-local-call-id","request":"client-request-id"}
{"op":"call.lease","conversation":"...","key":"64-hex-direct-key","callId":"16-hex-local-call-id","request":"start-or-answer-request-id"}
...
`version` is required (`omarchy plugin validate`). `import qs.Ui` / `qs.Commons`. No `/usr/share/omarchy/` edits.
...
Source installations use `omarchy plugin add <git-url> --yes` without requested enablement, then run the installed root `install.sh` to install dependencies, build the ignored local helper, enable OmaQ, and verify plugin IPC plus the matching helper after reactive activation and any in-flight watched-tree reload, without forcing a second shell restart. A future packaged activation path remains a separate Phase 7 design and must not discard or rebuild its packaged helper.
...
**Public.** Repository visibility and collaborator access remain owner-controlled. Do not change visibility. Do not add collaborators. Do not force-push to `main`.
