...
Source installations do not use `omarchy plugin add <git-url>`: its visible-tree move plus rescan precedes OmaQ's ignored local helper build. They use the external `install-omaq.sh` transaction. Any future package activation path remains a separate Phase 7 design and must not discard or reclone its packaged helper.
