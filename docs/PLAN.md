...
Settings (phase 4): `notifyBadge`, `notifyRightPanel`, `notifyDesktop`, `surfaceMode` (`separate`|`bundled`), `sound`, `soundCustomPath`, `chatTheme`, `animateUnread`. The compact panel keeps friend status dots, lists active groups with member counts beneath Friends, uses uniform larger action buttons, and arranges its tileless action icons in two vertical columns; selecting a listed group opens its chat directly.
