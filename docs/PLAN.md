...
A source update, helper backup, and normal helper build can each emit file-watch events inside the shared Quickshell process. Run the source update, helper build, and group-safe activation attempt before restarting the shell. The supported command must then attempt exactly one complete `omarchy restart shell` before checking network and OmaQ status. Its exit guard also attempts the restart after a source or helper failure. It skips helper work when the source update fails. After a successful restart, do not change monitored plugin files before the checks. Shell recovery must never depend on helper activation succeeding.

For updates from an already running Protocol-9-or-newer helper, the manual helper updater uses Service's existing detached helper-restart path rather than implementing a binary hot-swap. Before the normal `make helper` target replaces the available path, it descriptor-copies the verified image behind `/proc/<pid>/exe` to `helper/omaq.prev`. The runtime doctor binds PID, UID, start time, executable device/inode, socket device/inode, instance, and `SO_PEERCRED`, then compares the running and available SHA-256 values. Explicit activation sends only `helper.shutdown_if_no_groups`. A blocked, uncertain, or operation-unsupported result leaves the old process running with an explicit pending reason; a successful acknowledgement lets the normal socket-disconnect backoff start the current path. The doctor must then prove the replacement hash, executable inode, protocol marker, and correlated probe. A synchronous Makefile build or post-build validation failure immediately restores the available path from the verified `.prev` image while leaving the running process untouched. A later restart/post-check failure is a visible degraded state with manual `.prev` recovery. This intentionally has no staging build, hot-swap, signal fallback, timer, or crash-recovery journal.
