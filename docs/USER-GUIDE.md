...
Close the OmaQ panel with `Escape`, another press on the OmaQ bar action, or a click outside. The compact header intentionally has no redundant Close action, and no invisible input surface remains over the desktop.
...
8. Direct receipt markers use the active system palette: `·` in the foreground while sending, `✓` in `color05` when sent, `✓✓` in `color04` when delivered, and unframed `✓✓` in `color03` when read. Group messages report member-aware summaries such as **Read by 1** or **Delivered to 2**; one member's receipt never means everyone read the message. Delayed or replayed events never regress an existing Read state.
9. Use the `format_size` panel action to preview and set chat-message text to `90%`, `100%`, `110%`, `120%`, or `140%`. Composer, controls, receipts, and member labels keep their normal size. A message containing only valid Unicode emoji sequences uses the fixed 56-pixel emoji presentation even when the emoji was pasted and is not in OmaQ's picker.
