...
- Every live or packaged runtime payload includes executable `scripts/float-omaq.sh` and `scripts/uninstall-omaq.sh`; without the first, Hyprland cannot reliably float newly mapped OmaQ chat and demo windows. The uninstall wrapper must state that plugin removal retains private data, local state, `~/Downloads/omaq/`, optional deployment backups, dependency packages, and any Omarchy plugin backup, and must show that each retained directory can be inspected and deleted manually later.
...
- Chat message scaling uses only the fixed `90%`, `100%`, `110%`, `120%`, and `140%` steps and changes message-body text only. Composer, controls, receipts, and group member labels retain normal sizing. Valid emoji-only Unicode sequences render at the fixed 56-pixel emoji size whether they came from the picker or paste; mixed text remains ordinary message text. The settings panel shows a live text preview.
