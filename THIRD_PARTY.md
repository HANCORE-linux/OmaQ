...
| PulseAudio simple client library (`libpulse-simple`, provided by Arch `libpulse`) | bounded microphone capture and speaker playback in `helper/av.c` | LGPL-2.1-or-later | direct voice calls; PipeWire-Pulse provides the desktop audio server |
...
Installed here: `toxcore` 1:0.2.22-2; `libsignal-protocol-c` 2.3.3-2; `libpulse` provides `libpulse-simple` (`omarchy pkg add toxcore libsignal-protocol-c libpulse ttf-material-symbols-variable`).
